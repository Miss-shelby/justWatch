import { Text, View, TextInput, Pressable, KeyboardAvoidingView, Platform, Alert, ScrollView } from "react-native";
...
//  const { scrollHandler } = useAnimatedHeader();
 ...
 const { success, data: validatedData, error: validationError } = signInSchema.safeParse(formData);
          ...
          <View className="flex-1 w-full items-center px-2">
            ...
            <View className="mt-14 w-full">
                <View className="w-full">
                    <TextInput 
                        ...
                        {touched.email && fieldErrors.email?.[0] && (
                            <Text className="text-red text-sm mt-1">{fieldErrors.email[0]}</Text>
                        )}
                </View>
                <View className="w-full mt-6 relative">
                    <TextInput 
                        ...
                        {touched.password && fieldErrors.password?.[0] && (
                            <Text className="text-red text-sm mt-1">{fieldErrors.password[0]}</Text>
                        )}
                ...
                <View className="mt-6 flex-row items-center justify-center gap-2">
                    ...
                    <Link className="text-red" href="/(auth)/signUp">SignUp</Link>
                </View>
            </View>
          </View>
        </KeyboardAvoidingView>
